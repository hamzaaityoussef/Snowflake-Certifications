...
select  INITCAP(replace(replace(relative_path, '_', ' '), '.png')) as product_name , ts.FILE_NAME , ts.color_or_style , ts.price , ds.file_url
from directory(@sweatsuits) as ds
join zenas_athleisure_db.products.sweatsuits as ts
on ds.relative_path = ts.FILE_NAME );
